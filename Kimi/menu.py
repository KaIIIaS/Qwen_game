...
class Menus:
    ...
    def draw_help(self, surface):
        W, H = settings.SCREEN_WIDTH, settings.SCREEN_HEIGHT
        draw_text(surface, "СПРАВКА", 62, W // 2, int(H * 0.11), COLOR_WHITE, glow=True, glow_color=COLOR_CYAN)
        panel = pygame.Rect(int(W * 0.16), int(H * 0.18), int(W * 0.68), int(H * 0.59))
        draw_panel(surface, panel, (10, 18, 35), 225, (45, 115, 165), 18, 2)
        rows = [
            ("ДВИЖЕНИЕ", "Мышь или WASD / стрелки"),
            ("ОГОНЬ", "Автоматический, усиливается от уровня оружия"),
            ("SHIFT", "Точный режим: медленнее, зато проще маневрировать"),
            ("ПРОБЕЛ", "Бомба: очищает вражеские снаряды и наносит урон"),
            ("ESC", "Пауза во время боя, назад в меню"),
            ("КОМБО", "Убийства без пауз повышают множитель до x5"),
            ("ЩИТ", "Поглощает одно попадание"),
            ("МАГНИТ", "Притягивает бонусы к кораблю"),
            ("БОССЫ", "Меняют фазы и атаки по мере потери здоровья"),
        ]
        left = panel.left + 34
        right = panel.left + 265
        y = panel.top + 32
        row_h = 43
        for label, text in rows:
            draw_text(surface, label, 21, left, y, COLOR_CYAN, center=False)
            draw_text(surface, text, 20, right, y, (215, 220, 235), center=False)
            y += row_h
        for b in self.buttons:
            b.draw(surface)
